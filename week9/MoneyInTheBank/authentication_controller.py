...
class AuthenticatonController:
    ...
    def __is_registered(self, username):
        try:
            user = self.__session.query(Client).filter(
    ...
    def __success_login_attempt(self, user):
        user.login_attempts.append(
            LoginAttempt(attempt_status=LoginAttempt.SUCCESSFUL_ATTEMPT))
        self.__commit_changes([user])

    def __failed_login_attempt(self, user):
        user.login_attempts.append(
            LoginAttempt(attempt_status=LoginAttempt.FAILED_ATTEMPT))
        self.__commit_changes([user])

    def __block_user(self, user):
        user.is_blocked = True
        delta = datetime.timedelta(
            minutes=self.__settings["block_for_n_minutes"])
    ...
    def __unblock_user(self, user):
        ...
        user.login_attempts.append(
            LoginAttempt(attempt_status=LoginAttempt.AFTER_BLOCK, user_id=user.id))
        self.__commit_changes([user])
    ...
    def __can_login(self, user):
        ...
        login_attemps = self.__session.query(LoginAttempt).filter(
            LoginAttempt.user_id == user.id).all()
        last_n_failed = [
            a.attempt_status == LoginAttempt.FAILED_ATTEMPT for a in login_attemps[-block_after_n_logins:]]
